...
class Tokenizer {
  ...
  public async tokenize(texts: string[], maxInputLength = 48) {
    const all_encoded = new Array()
    const encoded_lengths = new Array<number>()
    texts.forEach(text => {
      all_encoded.push(this.tokenizer.tokenize(text).slice(0, maxInputLength))
    })
    all_encoded.forEach(encoded => {
      encoded_lengths.push(encoded.length)
    })
