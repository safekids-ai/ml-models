...
const Title = styled.span.withConfig({
  ...
  color: #4a4a4a;
  margin-bottom: '65px';
  margin-left: ${(props) => (props.isMobile ? '80px' : 'initial')};
`;
...
const WebsiteSettings = () => {
      ...
      putRequest<{}, { message: string; orgUnitId?: string }>(`${PUT_KID_REQUEST}?access-code=${kidRquestCode}`, {})
        .then((response) => {
          ...
          if (response.status === 200) {
            ...
            showNotification({
              message: response.data.message || 'Requested website has been added to always allowed list',
              type: 'success'
            });
          }
          getOrgunit();
        ...
        .catch((err) => {
          showNotification({
            message: err.response.data.message || 'Unable to requested website to always allowed list',
            type: 'error'
          });
          getOrgunit();
        });
  ...
  const getCategories = () => {
    if (accountType === 'SCHOOL')
      ...
      return (
        <SettingContainer>
          <CategoriesStatus refreshCategories={refreshCategories}
                            unSetRefreshCategories={() => setRefreshCategories(false)}/>
        </SettingContainer>
      );
    else return <></>;
  };
