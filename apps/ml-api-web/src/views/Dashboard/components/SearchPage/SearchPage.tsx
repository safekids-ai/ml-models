  ...
  return (
    <Root>
      ...
      <Autocomplete
        ...
        autoComplete
        autoHighlight
        // isOptionEqualToValue: No need to check for string values since options are strictly typed
        ...
        filterOptions={(options, state) => {
          ...
          return options.filter((option) =>
            ...
            `${option.firstName.toLowerCase()} ${option.lastName.toLowerCase()}`.includes(text)
          );
        }}
        options={options}
        loading={isLoading}
        // onInputChange: Optional, only needed if you handle input changes
        onInputChange={onTextChange}
        // renderOption: No need to handle strings, only AutoCompleteOption type
        ...
        {!isSearching && !!data.length && (
          <TableContainer>
            <div className="header">
              ...
              <Button className="export-button" onClick={onDownload} variant="contained"
                      disabled={!data.length || isSearching}>
                EXPORT RESULTS
              </Button>
            </div>
            <TableView OnPageChange={handlePageChange} rows={data} columns={generateColumns(data[0])} count={count}
                       currentPage={page}/>
          </TableContainer>
        )}
      </Content>
