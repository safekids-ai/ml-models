...
import { makeStyles, IconButton, Button, TextField, Switch, useTheme, Tooltip } from '@material-ui/core';
...
export const SelectField: React.FC<SelectProps> = (props: SelectProps) => {
    const { className, showClear = false, name, renderValue, variant = 'outlined', multiple = false, onKeyPress, ...rest } = props;
...
const PasswordStrengthBar: React.FC<PasswordStrengthBarProps> = ({ password, barCount = 8, onStrengthChange }: PasswordStrengthBarProps) => {
    ...
    for (let i = 0; i < barCount; i++) {
        bars.push(<div key={i} className={`password-strength-bar-item ${score > i * (100 / barCount) ? 'colored' : ''}`}></div>);
    }
...
export const PasswordField: React.FC<PasswordProps> = (props: PasswordProps) => {
    const { className, name, label, showStrengthBar = false, onStrengthChange, removeSpaces = false, showCriteria = false, infoStyle = {}, ...rest } = props;
                ...
                return (
                    <div style={{ position: 'relative' }}>
                        <TextField
                            ...
                            InputProps={{
                                endAdornment: (
                                    <IconButton aria-label="toggle password visibility" onClick={() => setVisible(!visible)} className="toggle-visibility">
                        ...
                        />
                        {showStrengthBar && (
                            <PasswordStrengthBar className="password-strength-bar" password={fieldProps.field.value} onStrengthChange={onStrengthChange} />
                        )}
                        {showCriteria && (
                            <div
                                style={{
                                ...
                                <Tooltip
                                    ...
                                    title={
                                        <div style={{ fontSize: '11px' }}>
                                            Your password should be between 8-20 characters and must be a combination of small letters, capital letters and
                                            numbers.
                                            <br />
...
export const SubmitButton: React.FC<ButtonProps> = (props: ButtonProps) => {
    const { text = 'Submit', className = '', isSubmitting = false, disabled = false, marginTop = '70px', marginBottom, onClick, id } = props;
...
export const MessageContainer: React.FC<MessageContainerProps> = ({ message, status, height = '70px' }: MessageContainerProps) => {
