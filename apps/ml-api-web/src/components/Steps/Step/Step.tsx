...
const Icon = styled.div<{
    ...
    display: flex;
    border: 1px solid
        ${(props: any) => (!props.complete && !props.active ? '#B5B5B5' : props.active ? (props.consumer ? '#FA6400' : '#F84260') : 'transparent')};
    border-radius: 4px;
    height: 45px;
    width: 40px;
    justify-content: center;
    align-items: center;
    background-color: ${(props: any) => (props.complete ? (props.consumer ? '#FA6400' : '#F84260') : 'white')};
`;

const Next = styled.span<{ active: boolean; consumer: boolean | undefined }>`
    color: ${(props: any) => (!props.active ? '#C1C7D0' : props.consumer ? '#FA6400' : '#f7274a')};
`;
