...
export class PostmarkEmailService implements EmailServiceInterface, QueueServiceInterface {
    ...
    async sendPostmarkEmail(email: EmailInterface): Promise<postmark.Models.MessageSendingResponse> {
        ...
        this.log.debug('Postmark email send', { email, params: params });
        ...
        try {
            const result = await retry(
                ...
                },
                {
                    ...this.retryOptions,
                    onRetry: (error: Error) => {
                        this.log.warn('Postmark Email Service OnRetry - retrying sending SES email due to', error);
                    },
                }
            );
            ...
            this.log.error('Postmark Email Service. Unable to send email message SES', error);
        }
    }
