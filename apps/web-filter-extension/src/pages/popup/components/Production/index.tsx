...
import {
  ...
  setPrr1Limit,
  setPrr2Threshold
} from '@shared/redux/actions/settings/index';
import {RootState} from '@shared/redux/reducers/index';
import {SettingsState} from '@shared/redux/reducers/settings';
import {PRRThreshold} from '@shared/redux/store';

import {Container, DropdownRow} from '@pages/popup/components/Production/styles';
...
export const Production: React.FC = () => {
  ...
  useEffect(() => {
    async function loadLanguage() {
      ...
      let value = await ChromeCommonUtils.readLocalStorage('language').catch((e) => {
        logger.warn(`Failed to read language. ${e}`);
      });
      if (value) {
        setLanguage(value);
      }
  ...
  return (
    <Container>
      <DropdownRow>
        <span>Language</span>
        <Select id={"languageField"} value={language?.id ? language.id : 'en'}
      ...
      <DropdownRow>
        <span>Images</span>
        <Select id={"imageAnalyzeId"} defaultValue={imageAnalyzeLimit} style={{width: 70}}
                onChange={(value) => dispatch(setImagesAnalyzeLimit(value))}>
      ...
      <DropdownRow>
        <span>Text </span>
        <Select id={"nlpAnalyzeId"} style={{width: 70}} defaultValue={nlpAnalyzeLimit}
                onChange={(value) => dispatch(setNLPAnalyzeLimit(value))}>
      ...
      <span>Image</span>
      Min:
      <Input data-testid={"minMLField"} value={minML} type="text" style={{width: '50px'}}
             onChange={(event) => setMinMl(event.target.value as unknown as number)}/>
      Max:
      <Input data-testid={"maxMLField"} value={maxML} type="text" style={{width: '50px'}}
             onChange={(event) => setMaxML(event.target.value as unknown as number)}/>
      <span>NLP</span>
      Min:{' '}
      <Input data-testid={"minNLPField"} value={minNLP} type="text" style={{width: '50px'}}
             onChange={(event) => setMinNLP(event.target.value as unknown as number)}/>
      Max:{' '}
      <Input data-testid={"maxNLPField"} value={maxNLP} type="text" style={{width: '50px'}}
             onChange={(event) => setMaxNLP(event.target.value as unknown as number)}/>
      <br/>
      <div>
      ...
      <DropdownRow>
        ...
        <Select id={"prr1LimitField"} defaultValue={prr1LimitValue} style={{width: 150}}
                onChange={(value) => dispatch(setPrr1Limit(value))}>
          <Option value="1">1</Option>
      ...
      <DropdownRow>
        ...
        <Select id={"prr2ThresholdField"} defaultValue={prr2ThresholdValue} style={{width: 150}}
                onChange={(value) => dispatch(setPrr2Threshold(value))}>
          <Option value="2">2</Option>
          <Option value="5">5</Option>
          <Option value="10">10</Option>
      ...
      <DropdownRow>
        ...
        <Select id={"informEventVisitsLimit"} defaultValue={informEventVisitsLimitVal} style={{width: 150}}
                onChange={(value) => dispatch(setInformEventVisitsLimit(value))}>
      ...
      <DropdownRow>
        ...
        <Select id={"informEventTimeoutLimit"} defaultValue={informEventTimeoutLimitVal} style={{width: 150}}
                onChange={(value) => dispatch(setInformEventTimeoutLimit(value))}>
