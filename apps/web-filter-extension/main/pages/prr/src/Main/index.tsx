...
import {aiTakeMeBack, getRandomLanguageMessage, triggerInformMessage} from '@src/Main/common';
import LimitAccessScreen from '@src/Main/PrrLevel2/limitAccessScreen';
import PrrLevel2 from '@src/Main/PrrLevel2/PrrLevel2';
import Triggers from '@src/Main/Triggers';
import {ButtonsSection, TextSection} from '@src/Main/main.style';
        ...
        {checkTriggerCondition ? (
          ...
          <>
            ...
            <section className="section text">
              <div lang={language.id} className="container">
                {level === PrrLevel.THREE && category !== PrrCategory.SELF_HARM_SUICIDAL_CONTENT && (
                  <div dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                       className="It-looks-like-youre">
                    {weapon1Message}
                    <br/> <br/>
                    {weapon2Message}
                  </div>
                ...
                {level === PrrLevel.THREE && category === PrrCategory.SELF_HARM_SUICIDAL_CONTENT && (
                  <>
                    <div dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                         className="Everyone-needs-some">
                      {getRandomLanguageMessage('prr3_selfHarm_1')}
                    </div>
                    <div dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                         className="Please-talk-to-teachers">
                      {getRandomLanguageMessage('prr3_selfHarm_2')}:
                    </div>
                    <div dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                         className="National-Suicide-Pre">
                    ...
                    <div dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                         className="Or-talk-to-one-of-yo">
                ...
                {showScreen2 && level === PrrLevel.ONE && (
                  <div dir={language.direction} lang={language.id} id="screen2content" className="more-info">
                    {category === PrrCategory.ADULT_SEXUAL_CONTENT && (
                      <p dir={language.direction} style={{textAlign: 'start'}} lang={language.id}
                         className="explicit-content-p">
                        {explicitPart1Text}
                        <br/> <br/>
                        {explicitPart2Text}
                      </p>
            ...
            {showScreen2 && level === PrrLevel.ONE && ai === 'true' && (
              <ButtonsSection className="m-t-26">
                <div lang={language.id} className="prr2-buttons">
                  <a id="take-me-back" onClick={() => aiTakeMeBack(ai, status, category, host, level)}
                     className="btn btn-light">
                    No
                  </a>
                  <a id="continue-trigger" onClick={() => triggerInformMessage(host, category)}
                     className="btn btn-dark btn-light">
                    Yes
                  </a>
                </div>
              </ButtonsSection>
          ...
          <PrrLevel2 language={language} level={level} category={category} siteName={host}
