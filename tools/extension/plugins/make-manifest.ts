...
let distDir = resolve(rootDir, 'dist');
let manifestFile = resolve(rootDir, 'manifest.js');
...
export default function makeManifest(config?: { getCacheInvalidationKey?: () => string}, distDirOverride?: string, manifestFileOverride?: string): PluginOption {
  if (distDirOverride) {
    distDir = distDirOverride;
  }
  if(manifestFileOverride) {
    manifestFile = manifestFileOverride;
  }
